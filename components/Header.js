...
import Avatar from './Aatar'
import HeaderOptions from './HeaderOptions'
import { MicrophoneIcon, SearchIcon, XIcon } from "@heroicons/react/solid";

function Header() {
  const router = useRouter();
  const [query, setQuery] = useState(router.query.term)
  ...
  return (
  <header className="flex flex-col justify-center sticky top-0 bg-white">
    <div className="w-full flex  items-center p-5 lg:py-5 lg:px-[60px]" >
        <div className="w-48 h-16 py-4  lg:-ml-8">
          <Image
            height={50}
            width={150}
            onClick={()=>router.push('/')}
      ...
      <form className="flex mx-4 max-w-md -ml-5 my-2 border border-gray-200 rounded-full flex-grow shadow-lg p-3">
        <input
          type="text"
          placeholder="Search"
          className="flex-grow w-full focus:outline-none px-4"
          value={query}
      ...
      <Avatar className="ml-auto" url={`https://images.unsplash.com/photo-1613250432964-46d07324734e?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=634&q=80`}/>
      </div>
      <HeaderOptions/>
    </header>
  );
